...
- `/pixiv` 备用命令
- 自然语言关键词优先整理为日文 Pixiv 标签，通过 Lolicon 进行 SFW 随机候选发现（不受收藏门槛限制），再由 Pixiv 获取原图；无候选时回退 Pixiv 标签搜索并强制附加收藏门槛
- 可配置强制收藏门槛：100 至 100000 users入り
- 可选画师 User ID 随机选图
- 可选指定插画 ID 或 Pixiv 作品链接发送
- R-18 / R-18G 默认过滤
- 图片临时下载、Pixiv 原图 URL 与官网 Referer、图片源校验、单文件大小限制和发送后自动清理
